...
struct passenger{
    char name[20],code[20],phone[20],id[20],mail[20];
    struct passenger* next;
};
struct passenger* head=NULL;
struct passenger* now=NULL;

void save_hash_to_file(const BYTE hash[], const char *id) {
    char filename[100];
	//char directory[] = "/";
    snprintf(filename, sizeof(filename), "hash_value_%s.txt", id);
...
int main()
{
	...
	//struct passenger passenger1 = {.id = "11"};
	//now = &passenger1;

    if (now != NULL) {// 您需要获取当前乘客的指针
        save_hash_to_file(hash, now->id);
    } else {
        printf("无法获取当前乘客信息\n");
    }
