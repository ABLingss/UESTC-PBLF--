...
int insert_user(sqlite3* db, struct passenger* user) {
    char* err_msg = 0;
    const char* tail;
    sqlite3_stmt* stmt;
...
int test_user(sqlite3* db, const char* username, const char* password) {
    char* err_msg = 0;
    const char* tail;
    sqlite3_stmt* stmt;
...
int date_input(const char* input, int max_len) {
    if (input == NULL) {
        return 0;
    }
