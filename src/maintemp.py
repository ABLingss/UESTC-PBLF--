...
class LoginWindow(QtWidgets.QMainWindow, LoginUi):
    ...
    def login(self):
        ...
        if name == '' or password == '':
            ...
            result = passenger_lib.login(name.encode('utf-8'), password.encode('utf-8'))
    ...
    def register(self):
        ...
        if name == '' or password == '' or phone == '' or id == '' or mail == '':
            print("请填写完整信息")
        else:
            # 调用 C 语言的 regis 函数进行注册
            result = passenger_lib.regis(name.encode('utf-8'), password.encode('utf-8'), phone.encode('utf-8'),
